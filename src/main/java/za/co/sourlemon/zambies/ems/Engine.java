...
public class Engine implements EntityListener
{
    ...
    /**
     * Adds an entity to the list of known entities. Automagically creates
     * and relevant node(s) which is/are added to the appropriate node list.
     * This method may be called at any time as entities will only be physically
     * added to the list once any update loop is completed. This allows systems
     * to add entities during their execution without worrying about
    ...
    /**
     * Gets the first instance of the specified node class. This is a convenience
     * method for getting instances of nodes of which only one is expected to
     * exist. (Sort of like the interface to a kind of "singleton" node.
     * @param <N> the Node Class type
     * @param nodeClass the node class
