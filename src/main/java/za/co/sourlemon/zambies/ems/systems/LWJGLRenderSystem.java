...
public class LWJGLRenderSystem extends AbstractSystem
{
    ...
    public void update(double delta)
    {
        ...
        events.mouse.x = Mouse.getX() - camX;
        events.mouse.x = Mouse.getY() - camY;
        ...
        GL11.glLoadIdentity();
        GL11.glOrtho(camX - SCREEN_WIDTH / 2, camX + SCREEN_WIDTH / 2, camY + SCREEN_HEIGHT / 2, camY - SCREEN_HEIGHT / 2, 1, -1);
        ...
        for (RenderNode node : nodes)
        {
            ...
            drawQuad(p.x, p.y, p.theta, p.scaleX, p.scaleY, c[0], c[1], c[2]);
        }

        Display.update();
    }

    public void drawQuad(float x, float y, float theta, float scaleX, float scaleY, float r, float g, float b)
    {
        GL11.glPushMatrix();
